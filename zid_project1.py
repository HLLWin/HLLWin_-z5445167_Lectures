...
TICPATH = os.path.join(ROOTDIR, "lectures/Week 4/TICKERS.txt")
...
def get_tics(pth):
    ...
    """
# Initialize an empty dictionary to store tickers and exchanges
    ticker_dict = {}

# Open the file specified by pth
    with open(pth, 'r') as file:
        for line in file:
# Remove leading and trailing whitespace
            line = line.strip()
        if line:
# Split the line into exchange and ticker
            exchange, ticker = line.split('=')

# Format ticker and exchange according to the rules
        formatted_ticker = ticker.lower()
        formatted_ticker = ''.join(char for char in formatted_ticker if char.isalpha())
        formatted_exchange = exchange.lower()
        formatted_exchange = ''.join(char for char in formatted_exchange if char.isalpha())

# Check if ticker and exchange are not empty
        if formatted_ticker and formatted_exchange:
# Add formatted ticker and exchange to the dictionary
            ticker_dict[formatted_ticker] = formatted_exchange
...
def read_dat(tic):
    ...
    # <COMPLETE THIS PART>
# Define the full path to the data file for the specified ticker
    dat_file_path = os.path.join(DATDIR, f'{tic}.dat')

    # Initialize an empty list to store the lines of the data file
    lines = []
    ...
    if os.path.exists(dat_file_path):
        ...
        with open(dat_file_path, 'r') as file:
            # Read each line and append it to the list (without newline characters)
            lines = [line.rstrip('\n') for line in file]
...
# Initialize an empty dictionary to store the column values
    ...
    # Initialize a starting index to extract data from the line
    start_index = 0

    # Iterate over the columns and extract data from the line
    for column, width in zip(COLUMNS, COLWIDTHS):
        # Extract the data for the current column
        data = line[start_index:start_index + width].strip()
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    """
    # <COMPLETE THIS PART>
# Rule 1: If tickers_lst is an empty list, raise an Exception
    if tickers_lst is not None and not tickers_lst:
        raise Exception("tickers_lst is an empty list.")

    # Rule 2: If tickers_lst contains a ticker that does not correspond to a key in tic_exchange_dic
    if tickers_lst is not None:
        invalid_tickers = [ticker for ticker in tickers_lst if ticker.lower() not in tic_exchange_dic]
        if invalid_tickers:
            raise Exception(f"The following tickers are not valid: {', '.join(invalid_tickers)}")
...
def verify_cols(col_lst=None):
    ...
    """
    # <COMPLETE THIS PART>
 # Rule 1: If col_lst is an empty list, raise an Exception
    if col_lst is not None and not col_lst:
        raise Exception("col_lst is an empty list.")

    # Rule 2: If col_lst contains a column that is not found in COLUMNS
    if col_lst is not None:
        invalid_columns = [col for col in col_lst if col.lower() not in [c.lower() for c in COLUMNS]]
        if invalid_columns:
            raise Exception(f"The following columns are not valid: {', '.join(invalid_columns)}")
 ...
 # Check if tickers_lst contains any invalid tickers
    ...
    for ticker in tickers_lst:
        ...
        for line in lines:
            ...
            # Filter data_dict to only contain columns listed in col_lst
            if col_lst is not None:
                data_dict = {col: data_dict[col] for col in col_lst}

            data.append(data_dict)

        result_dict[ticker] = {'exchange': exchange, 'data': data}
