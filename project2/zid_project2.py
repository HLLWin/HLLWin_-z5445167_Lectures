...
def read_prc_csv(tic):
    ...
    """
    # <COMPLETE THIS PART>
    data_path = os.path.join(cfg.DATADIR,f'{tic.lower()}_prc.csv')
...
def mk_prc_df(tickers, prc_col='adj_close'):
    ...
    **not** necessarily represent the actual contents of the data frame you
    ...
    """
    # <COMPLETE THIS PART>
    prc_col_df = pd.DataFrame()
    for ticker in tickers:
        data_slice = read_prc_csv(tic=ticker)[[prc_col]].copy()
        data_slice.columns = [ticker]
        prc_col_df = pd.concat([prc_col_df,data_slice],axis = 1)
...
def mk_ret_df(prc_df):
    ...
    """
    # <COMPLETE THIS PART>
    prc_df_copy = prc_df.copy().sort_index()

    price_return_df = pd.DataFrame()
    for ticker in prc_df_copy.columns:
        price_return_ticker = prc_df_copy.copy()[[ticker]].dropna().pct_change()
        price_return_ticker = price_return_ticker.sort_index()
        price_return_df = pd.concat([price_return_df,price_return_ticker],axis = 1)
    price_return = price_return_df.copy()



    market_data = pd.read_csv(cfg.FF_CSV, index_col='Date', parse_dates=['Date'])
    market_return = market_data[['mkt']]
    ret_df = price_return.join(market_return,how = 'left')
...
def mk_aret_df(ret_df):
    ...
    aret_df = ret_df.copy().drop(columns=['mkt']).sub(ret_df.copy()['mkt'], axis=0)
    return aret_df
...
def get_avg(df, col, year):
    ...
    new_df = new_df.loc[str(year),col]
    avg_df = new_df.mean(axis=0, skipna=True)
    return avg_df
...
def get_ew_rets(df, tickers):
    ...
    """
    #<COMPLETE THIS PART>
    avg_df = df.loc[:,tickers].mean(axis=1,skipna=True)
    return avg_df
...
def get_ann_ret(ser, start, end):
    ...
    daily_return = ser.dropna()
    daily_return_slice = daily_return.loc[start:end]
    tot_ret = (1 + daily_return_slice).prod()
    N = len(daily_return_slice)
    result = (tot_ret)**(252/N) - 1
    return result
