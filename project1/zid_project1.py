...
def get_tics(pth):
    ...
    with open(pth) as file:
        for line in file.readlines():
            if len(line.strip()) > 0:
                line = line.strip()
                line = line.lower()  # changing lower case
                # line -> "XXXX"="YYYY
                tmp = line.split('=')
                # tmp -> {"XXXX", "YYYY"]
                exchange = tmp[0].strip('"')  # removing double quotes
                tic = tmp[1].strip('"')
...
def read_dat(tic):
    ...
    # Construct the path to the .dat file
    ...
    # Initialize an empty list to store selected columns (tic and prc)
    selected_data = []

    tickers_lst = []  # Assuming you have a list of tickers here
    ...
    for tic in tickers_lst:
        ...
        for line in data:
            if len(line.strip()) > 0:
                ...
                # tmp -> {"XXXX", "YYYY"]
                tic = tmp[1].strip('"')
                prc = line.split()[3].strip()
                selected_data.append((tic, prc))
            else:
                pass
    print(selected_data)
...
def line_to_dict(line):
    ...
    for col in COLUMNS:
        ...
        if col == 'Date':  # Check for 'Date' column
            data_dict[col] = value  # Keep as string for 'Date'
        elif col in ['Volume', 'Adj Close', 'Close', 'Open', 'High']:
            data_dict[col] = float(value)  # Convert to float for specified columns
        else:
            data_dict[col] = value  # For other columns, keep as string
...
def verify_tickers(tic_exchange_dic, tickers_lst=None):
    ...
    # Using the get_tics function to create the dictionary
    tic_exchange_dic = get_tics(TICPATH)

    # Using the read_dat function to read data for a specific ticker
    data = read_dat('aapl')

    # Checking if there is data available
    if data:
        # Using the line_to_dict function to convert a line to a dictionary
        line_data = line_to_dict(data[0])
        print(line_data)
    else:
        pass

    # Verifying tickers using verify_tickers
    verify_tickers(tic_exchange_dic, tickers_1st)
    ...
    if tickers_lst is not None:
        ...
        for ticker in tickers_lst:
            if ticker.lower() not in tic_exchange_dic:
                raise Exception(f"'{ticker}' is not a key of tic_exchange_dic.")
...
def verify_cols(col_lst=None):
    ...
    if col_lst is not None:
            # Rule 1: col_lst is an empty list
        if not col_lst:
            raise Exception("col_lst is an empty list")

        # Rule 2: col_lst contains a column that is not found in COLUMNS
        for col in col_lst:
...
def create_data_dict(tic_exchange_dic, tickers_lst=None, col_lst=None):
    ...
    for tic in tickers_lst:
        ...
        if col_lst is None:
            new_data = [line_to_dict(line) for line in data]
        else:
            new_data = [line_to_dict(line) for line in data if line_to_dict(line).keys() & col_lst]

        tic_exchange = tic_exchange_dic[tic.lower()]

        data_dict[tic] = {'exchange': tic_exchange, 'data': new_data}
...
def create_json(data_dict, pth):
    ...
    create_json(data_dict, 'output.json')
